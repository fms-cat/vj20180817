...
void main() {
  ...
  // == update particles =======================================================
  vel.xyz += 10.0 * noiseScale * vec3(
    noise( vec4( 0.9 * pos.xyz, 1.485 + sin( time * 0.1 ) + noisePhase ) ),
    noise( vec4( 0.9 * pos.xyz, 3.485 + sin( time * 0.1 ) + noisePhase ) ),
    noise( vec4( 0.9 * pos.xyz, 5.485 + sin( time * 0.1 ) + noisePhase ) )
  ) * dt;
  // vel.y += 10.0 * dt;
